          ...
          <div className="flex items-center gap-8">
            ...
            <label className="hidden md:flex flex-col min-w-40 h-10 w-64 lg:w-80">
              <div className="flex w-full flex-1 items-stretch rounded-full h-full bg-[#16161e] border border-[#2a2a3a] overflow-hidden focus-within:ring-2 ring-primary/50 transition-all">
                ...
                <input
                  className="flex w-full bg-transparent border-none text-sm font-medium text-white focus:outline-0 focus:ring-0 placeholder:text-gray-500 h-full"
            ...
            <div className="flex gap-3 relative">
              ...
              {loading ? (
                ...
                <div ref={dropdownRef} className="relative group">
                  ...
                  {/* Dropdown Menu */}
                  <div
                    className={`absolute right-0 top-full mt-2 w-64 bg-[#16161e] border border-[#2a2a3a] rounded-2xl shadow-xl overflow-hidden transition-all duration-200 origin-top-right ${isDropdownOpen ? "opacity-100 scale-100 translate-y-0" : "opacity-0 scale-95 -translate-y-2 pointer-events-none"}`}
                  >
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <span
                              className={`text-[10px] font-bold px-1.5 py-0.5 rounded border ${
                                (user.role || user.Role || "").trim().toUpperCase() === "ADMIN"
                                  ? "bg-red-500/20 text-red-400 border-red-500/30"
                                  : (user.role || user.Role || "").trim().toUpperCase() === "INSTRUCTOR"
                                    ? "bg-blue-500/20 text-blue-400 border-blue-500/30"
                                    : "bg-emerald-500/20 text-emerald-400 border-emerald-500/30"
                              }`}
                            >
                              {(user.role || user.Role || "LEARNER").trim().toUpperCase()}
                            </span>
                          </div>
                    ...
                    <div className="p-2">
                      {(user.role || user.Role || "").trim().toLowerCase() === "instructor" && (
                        <>
                          <Link
                            to="/instructor-dashboard"
                            onClick={() => setIsDropdownOpen(false)}
                            className="flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-purple-500/10 text-purple-400 hover:text-purple-300 transition-colors"
                          >
                            <span className="material-symbols-outlined text-[20px]">
                              dashboard
                            </span>
